...
- 依据文档：
  - `oh-my-robot/lib/drivers/include/drivers/motor/doc/电机统一模型顶层设计规范.md`
...
| --- | --- | --- | --- | --- | --- |
| Phase 0（合同与边界冻结） | 进行中 | 已启动 | `oh-my-robot/lib/drivers/include/drivers/motor/doc/*.md` | 文档已重构，待一致性复核 | 合同到实现映射样例尚未落地 |
...
| 合同域 | 当前状态 | 证据路径 | 备注 |
| --- | --- | --- | --- |
| `MM-BUS` | 已定义 | `电机统一模型语义合同索引.md` | 待实现映射验证 |
| `MM-PROTO` | 已定义 | `电机统一模型语义合同索引.md` | 待样板适配验证 |
| `MM-CORE/CMD/STATE` | 已定义 | `电机统一模型语义合同索引.md` | 待 core 壳实现验证 |
| `MM-SCH/SAFE/FLT` | 已定义 | `电机统一模型语义合同索引.md` | 待回归验证 |
| `MM-PARAM/DIAG/ENG/CAP` | 已定义 | `电机统一模型语义合同索引.md` | 待阶段 3 收敛 |

## 2.2 命名合规检查

| 检查项 | 当前结论 | 证据方式 |
| --- | --- | --- |
| 公共类型名不含版本后缀（如 `...V1`） | 通过（文档层） | `rg -n "Motor.*V[0-9]+" oh-my-robot/lib/drivers/include/drivers/motor/doc` |
